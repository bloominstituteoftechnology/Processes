...
Sean lecture today went deeper into system calls such as wait(). He explained that the wait call prevents child processes from turning into zombie processes. Sean also talked about what both multiprocessing and multithreading are. He explained that multiprocessing folks new processes that have share the functions as its parent. This is can become very resource intensive since each copy takes up memory. Multithreading reads and wirtes in the same address spacing thus its memory footprint can be compacted. But the tradeoff is, its harder to write processes that are independent enough where they wouldnt refencence the same thing and potientaly write over one another. He finished with timespec. Overall I feel like I understood all the topics that he discussed today and this was also a short lecture so that was nice.

Sean expounded more on the wait() system call. That it basically cleans up child processes to prevent them from running perpetually or go into zombie state. He also explained the difference between Multiprocessing vs Multithreading. Multiprocessing duplicates full on copies of a certain process by calling the fork() function. The resulting clone possess the same functionalities and same codes as its parent process. This, however, is very resource intensive and would take up a lot of memory from RAM. Multithreading, on the other hand, read and write from a single address space. Though it is more lightweight, it also has some risks/complexities. Because of its sharing nature, the code is more complicated here. It also runs the high risk of overwriting some of the functions because they're all from a single address space. Lastly, he delved into timespec. Though the logic was explained, its syntax is a bit unfamiliar to me. I will need to study it more closely to better understand it.