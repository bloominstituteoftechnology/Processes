...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < num_processes; i++) {
		if (fork() == 0) {
			...
			// functions, above).
			int fw = open_balance_file(BALANCE_FILE);
			flock(fw, LOCK_EX);
			// Read the current balance
			read_balance(fw, &balance);
			...
			if (balance >= amount) {
				...
				balance -= amount;
				write_balance(fw, balance);
				printf("Old balance %d, Withdrew $%d, new balance $%d\n", old_balance, amount, balance);
			}
			else {
