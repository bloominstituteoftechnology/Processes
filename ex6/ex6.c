...
int main()
{
    ...
    for (int i=0; i<number_iter; i++){ // loops through 1 million times
      clock_gettime(CLOCK_MONOTONIC, &start); // start the elapse timer
      fileno(stdout); // empty write to stdout, per Sean
...
//     printf("CLOCK_MONOTONIC elapsed process CPU time = %llu nanoseconds\n", (long long unsigned int) diff);
//     return 0;
// }

// SINGLE RUN - CLOCK_PROCESS_CPUTIME_ID (6k-7k nanosecs)
// int main()
// {
//     // Your code here
//     uint64_t diff;
...
//     diff = BILLION * (end.tv_sec - start.tv_sec) + end.tv_nsec - start.tv_nsec;
//     printf("CLOCK_PROCESS_CPUTIME_ID elapsed process CPU time = %llu nanoseconds\n", (long long unsigned int) diff);
//     return 0;
// }